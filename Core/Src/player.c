...
int Player_Play(int track_index) {
    ...
    AudioInfo info;
    if (current_decoder->open(&current_file, &info) != 0) {
...
void Button_Process(void) {
    ...
    // --- СОБЫТИЕ: УДЕРЖАНИЕ (Кнопка всё еще нажата) ---
    ...
    if (current_state == GPIO_PIN_SET && last_state == GPIO_PIN_RESET) {
        ...
        if (!hold_executed) {
            // Если мы НЕ удерживали кнопку для громкости, значит это просто короткий клик
            if (now - btn_press_time > 20) { // Защита от дребезга
                click_count++;
            }
        } else {
            // Если мы закончили изменять громкость, обнуляем серию кликов
            click_count = 0;
        }
    }
